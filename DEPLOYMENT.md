...
**`https://<custom-domain>/battler`** for **~$0/month**.
...
browser
  │
  ▼
<custom-domain>  ── Firebase Hosting (free managed SSL, CDN edge)
  ├─ /                → 302 → /battler/
...
| the `.xyz` registrar | domain + DNS records for `<custom-domain>` | domain resolves nowhere |
...
```json
{ "projects": { "default": "<gcp-project-id>" } }
```
...
Add `<custom-domain>` (and `www`) as a custom domain in Firebase Hosting. At
