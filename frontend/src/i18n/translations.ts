const en = {
  'app.title': 'Pokémon Auto Battler',

  'common.close': 'Close',
  'common.loading': 'Loading…',
  'common.levelAbbrev': 'L',
  'common.moreInfo': 'More info',

  'app.logout': 'Log out',
  'auth.welcomeHeading': 'The Poké Professor',
  'auth.welcomeQuestion':
    "Hi, I'm the Poké Professor — and you must be one of the battle experts. Is this... your first time here?",
  'auth.welcomeRegisterButton': 'Yes, nice to meet you! (Register)',
  'auth.welcomeLoginButton': "What, don't you remember me? (Login)",
  'auth.backButton': '‹ Back',
  'auth.registerHeading': 'Trainer Card',
  'auth.registerExplainer':
    'Welcome! We need all the helping hands we can get. I just need some very basic data about you to make sure your contributions are remembered! If you have any question about the data, just tap the {{iButton}} icon.',
  'auth.iButtonHint': 'Tap one of these next to any field to learn more about it.',
  'auth.loginHeading': 'Welcome Back',
  'auth.loginExplainer':
    'Enter your username and your **Poké-Password** — the same trio, in the same order, you picked when you registered.',
  'auth.usernameLabel': 'Username',
  'auth.usernameHelp': "This is your account name, not the name people will see — pick something easy for you to remember, since you'll type it every time you come back.",
  'auth.displayNameLabel': 'Display name',
  'auth.displayNameHelp': 'This is the name other trainers will see instead of your username. Pick whatever you like.',
  'auth.pokemonGroupLabel': 'Poké-Password',
  'auth.pokemonHelp':
    'This trio, in this exact order, is your password — a new username claims it, an existing one has to match it to get back in.',
  'auth.pokemonSlotLabel': 'Pokémon {{n}}',
  'auth.pokemonPlaceholder': 'Choose…',
  'auth.pokemonNoResults': 'No matches.',
  'auth.submit': 'Enter the Gym',
  'auth.error.generic': 'Could not log in.',
  'auth.error.speciesFailed': 'Failed to load the Pokémon list.',
  'auth.error.missingFields': 'Please fill in all the fields.',
  'auth.error.incompleteCombo': 'Pick all three Pokémon.',
  'auth.error.invalidPokemon': 'One of the selected Pokémon is not valid.',
  'auth.error.usernameTaken': 'That username is already taken.',
  'auth.error.invalidCredentials': 'Wrong username or Pokémon combination.',
  'auth.error.sessionStartFailed': 'Could not start your session. Please try again.',
  'auth.checkingSession': 'Checking your trainer card…',

  'intro.rulesIntro': 'Build your team before you walk in. The rules are strict:',
  'intro.rule.noItems': 'No held items',
  'intro.rule.noUsableItems': 'No usable items during battle',
  'intro.rule.pokemonCount': '{{count}} Pokémon only',
  'intro.rule.exclusiveStarter': 'Some Pokémon are mutually exclusive — you can only ever have one starter',
  'intro.rule.levelCap': 'Level cap: {{cap}}',
  'intro.rule.evoStage': 'Pick any evolution stage legal at level {{cap}}',
  'intro.rule.moves': 'Pick up to {{max}} moves legal at level {{cap}}',
  'intro.cta': 'Build Your Team',

  // Identity copy for each gym leader, kept out of the generic `intro.*`/
  // `battle.*` keys above so a second leader (Misty) is a namespace to add
  // rather than a set of hardcoded strings to find and branch on.
  'leader.brock.splashTitle': 'Breaking Brock',
  'leader.brock.heading': 'Pewter Gym',
  'leader.brock.rivalLabel': 'Gym Leader Brock',
  'leader.brock.description':
    'Waiting inside is **Brock**, the Rock-type Gym Leader — tough, patient, and about to test whether your team can actually take a hit.',
  /** Short display name, for `{{leader}}` interpolation in generic battle
   * copy and for translating the raw `|win|<label>` protocol line. */
  'leader.brock.name': 'Brock',

  'leader.misty.splashTitle': 'Misty Waters',
  'leader.misty.heading': 'Cerulean Gym',
  'leader.misty.rivalLabel': 'Gym Leader Misty',
  'leader.misty.description':
    "Waiting inside is **Misty**, the Water-type Gym Leader — quick, relentless, and ready to sweep away any team that can't keep its footing.",
  'leader.misty.name': 'Misty',

  'leaderBar.locked': 'Not yet available',
  'leaderBar.lockedInBuild': 'Go back to change leader',

  'teamBuilder.loadingRoster': 'Loading roster…',
  'teamBuilder.loadRosterFailed': 'Failed to load roster.',
  'teamBuilder.back': 'Back',
  'teamBuilder.heading': 'Build Your Team',
  'teamBuilder.importFromShowdown': 'Import from Showdown',
  'teamBuilder.cancelImport': 'Cancel Import',
  'teamBuilder.rules': 'Level cap {{cap}} · No items · {{count}} Pokémon only · pick an evolution stage and up to {{max}} moves legal at level {{cap}}.',
  'teamBuilder.importPlaceholderIntro': 'Paste a Showdown export, e.g.',
  'teamBuilder.import': 'Import',
  'teamBuilder.importing': 'Importing…',
  'teamBuilder.importFailed': 'Failed to import team.',
  'teamBuilder.pokemonSlot': 'Pokémon {{n}}',
  'teamBuilder.selectHeading': 'Choose Pokémon {{n}}',
  'teamBuilder.slotLocked': 'Locked for this challenge',
  'teamBuilder.removePokemon': 'Remove Pokémon',
  'teamBuilder.starterBlocked': 'Only one starter allowed on your team',
  'teamBuilder.tradeBlocked': 'Trading for one costs you the other',
  'teamBuilder.duplicateBlocked': 'Your team cannot contain the same Pokemon twice.',
  'teamBuilder.familyBlocked': 'Already on your team as a different evolution stage',
  'teamBuilder.starterValidation': 'Only one starter (Bulbasaur/Charmander/Squirtle) can be on your team.',
  'teamBuilder.tradeValidation': "Your team can't include both sides of an in-game trade (e.g. Clefairy and Mr. Mime).",
  'teamBuilder.familyValidation': "Your team can't contain two Pokemon from the same evolution family (e.g. Gloom and Vileplume).",
  'teamBuilder.abilityHeading': 'Ability',
  'teamBuilder.natureHeading': 'Nature',
  'teamBuilder.selectNature': 'Select nature',
  'teamBuilder.movesHeading': 'Moves',
  'teamBuilder.movesSelected': '{{count}} / {{max}} selected',
  'teamBuilder.battleCta': 'Battle!',

  'battle.loading': '{{leader}} sends out a team…',
  'battle.runFailed': 'Battle failed to run.',
  'battle.backToBuilder': 'Back to Team Builder',
  'battle.step': 'Next Move',
  'battle.skipToEnd': 'Skip to End',
  'battle.pause': 'Pause',
  'battle.play': 'Play',
  'battle.newTeam': 'Build a New Team',
  'battle.rematch': 'Fight Again',
  'battle.turnHeading': 'Turn {{n}}',
  'battle.turnHeading0': 'Send-out',
  'battle.outcome.tie': 'The battle ended in a tie.',
  'battle.outcome.win': 'You defeated {{leader}}!',
  'battle.outcome.lose': '{{leader}} defeated your team.',
  'battle.playerLabel': 'Red',

  'battle.entersBattleSuffix': ' enters the battlefield!',
  'battle.abilityActivatedSuffix': ' activated {{ability}}!',
  'battle.weatherBegins': '{{weather}} begins!',
  'battle.used': 'used',
  'battle.on': 'on',
  'battle.targetSelf': 'itself',
  'battle.targetAlly': 'its ally',
  'battle.targetAllFoes': 'all enemies',
  'battle.targetAllAdjacent': 'everyone nearby',
  'battle.targetField': 'everyone',
  'battle.targetFoeSide': "the rival's side",
  'battle.targetOwnSide': 'its side',
  'battle.targetTeam': 'its team',
  'battle.resistedPrefix': "It's not very effective on",
  'battle.resistedSuffix': '...',
  'battle.superEffectivePrefix': "It's super effective on",
  'battle.superEffectiveSuffix': '!',
  'battle.immuneSuffix': ' is immune to that move.',
  'battle.critPrefix': 'A critical hit on',
  'battle.critSuffix': '!',
  'battle.missPrefix': '',
  'battle.missSuffix': "'s attack missed!",
  'battle.failTargetPrefix': '',
  'battle.failTargetSuffix': "'s move failed!",
  'battle.failGeneric': 'But it failed!',
  'battle.tookDamageSuffix': ' took damage ({{amount}}).',
  'battle.restoredHpSuffix': ' restored HP ({{amount}}).',
  'battle.hpUnit': 'HP',
  'battle.boostRose': 'rose',
  'battle.boostFell': 'fell',
  'battle.statusWas': 'was',
  'battle.status.par': 'paralyzed',
  'battle.status.brn': 'burned',
  'battle.status.psn': 'poisoned',
  'battle.status.tox': 'badly poisoned',
  'battle.status.slp': 'put to sleep',
  'battle.status.frz': 'frozen solid',
  'battle.status.generic': 'afflicted with a status condition',
  'battle.curedSuffix': ' recovered from its status!',
  'battle.faintedSuffix': ' fainted!',
  'battle.winsSuffix': ' wins the battle!',
  'battle.tieLine': 'The battle ended in a tie!',

  'battle.moveDetail.power': 'Power',
  'battle.moveDetail.accuracy': 'Accuracy',
  'battle.moveDetail.pp': 'PP',
  'battle.moveDetail.priority': 'Priority',
  'battle.moveDetail.error': 'Failed to load move.',

  'battle.reportMove.button': 'Report this move',
  'battle.reportMove.title': 'Suggest a better move',
  'battle.reportMove.suggestionLabel': 'What should have happened instead?',
  'battle.reportMove.suggestionPlaceholder': 'e.g. Switch to the other Pokémon, or use a different move…',
  'battle.reportMove.reasonLabel': 'Why?',
  'battle.reportMove.reasonPlaceholder': 'e.g. That move was resisted, a teammate could have KO\'d instead…',
  'battle.reportMove.submit': 'Submit suggestion',
  'battle.reportMove.thanks': 'Thanks! Your suggestion was recorded.',
  'battle.reportMove.error': 'Failed to submit suggestion.',

  'footer.cta.0': 'Ah, a glitch? Tell me every last detail — for science!',
  'footer.cta.1': 'Something not working right? Report it like a true researcher!',
  'footer.cta.2': 'Even my finest research has flaws. Help me squash this one!',
  'footer.cta.3': 'Enjoying your journey so far? Tell me — it makes my day!',
  'footer.cta.4': 'A kind word does wonders in the lab. Go on, say it!',
  'footer.cta.5': 'Whether it made you laugh or groan, I simply must know!',
  'footer.cta.6': 'A brilliant idea? My notebook always has room for one more!',
  'footer.cta.7': "Dreaming up a new feature? I'm all ears — do share!",
  'footer.cta.8': 'What would make this journey even better? Enlighten me!',
  'footer.cta.9': 'Two seconds, one thought — humor an old professor!',
  'footer.button': 'Leave feedback',
  'footer.placeholder': 'Type your feedback here…',
  'footer.submit': 'Send feedback',
  'footer.thanks': 'Thanks! Your feedback was recorded.',
  'footer.error': 'Failed to submit feedback.',
  'footer.close': 'Close',

  'pokemonCard.natureNeutral': 'Neutral',
  'pokemonCard.itemHeading': 'Held Item',
  'pokemonCard.itemNone': 'None',
  'pokemonCard.statsHeading': 'Base Stats',
  'pokemonCard.viewDetails': 'View details for {{name}}',
};

const es: Record<keyof typeof en, string> = {
  'app.title': 'Pokémon Auto Battler',

  'common.close': 'Cerrar',
  'common.loading': 'Cargando…',
  'common.levelAbbrev': 'Nv.',
  'common.moreInfo': 'Más información',

  'app.logout': 'Cerrar sesión',
  'auth.welcomeHeading': 'El Profesor Poké',
  'auth.welcomeQuestion':
    'Hola, soy el Profesor Poké — y tú debes ser una de las personas expertas en combate. ¿Es esta... tu primera vez aquí?',
  'auth.welcomeRegisterButton': 'Sí, un placer conocerte (Registrarse)',
  'auth.welcomeLoginButton': '¿Cómo que no te acuerdas de mí? (Iniciar sesión)',
  'auth.backButton': '‹ Atrás',
  'auth.registerHeading': 'Carné Pokémon',
  'auth.registerExplainer':
    '¡Te doy la bienvenida! Necesitamos toda la ayuda que podamos conseguir. Solo necesito algunos datos muy básicos sobre ti para que tus contribuciones queden registradas. Si tienes alguna duda sobre estos datos, toca el icono {{iButton}}.',
  'auth.iButtonHint': 'Toca uno de estos junto a cualquier campo para saber más sobre él.',
  'auth.loginHeading': '¡Qué alegría verte de nuevo!',
  'auth.loginExplainer':
    'Introduce tu nombre de usuario y tu **Poké-Contraseña**: el mismo trío, en el mismo orden, que elegiste al registrarte.',
  'auth.usernameLabel': 'Nombre de usuario',
  'auth.usernameHelp':
    'Este es el nombre de tu cuenta, no el nombre que verá la gente — elige algo fácil de recordar, ya que lo escribirás cada vez que vuelvas.',
  'auth.displayNameLabel': 'Nombre visible',
  'auth.displayNameHelp': 'Es el nombre que verán los demás entrenadores en lugar de tu nombre de usuario. Elige el que quieras.',
  'auth.pokemonGroupLabel': 'Poké-Contraseña',
  'auth.pokemonHelp':
    'Este trío, en este orden exacto, es tu contraseña — un usuario nuevo la reserva, uno existente debe repetirla para volver a entrar.',
  'auth.pokemonSlotLabel': 'Pokémon {{n}}',
  'auth.pokemonPlaceholder': 'Elige…',
  'auth.pokemonNoResults': 'Sin resultados.',
  'auth.submit': 'Entrar al Gimnasio',
  'auth.error.generic': 'No se pudo iniciar sesión.',
  'auth.error.speciesFailed': 'No se pudo cargar la lista de Pokémon.',
  'auth.error.missingFields': 'Rellena todos los campos.',
  'auth.error.incompleteCombo': 'Elige los tres Pokémon.',
  'auth.error.invalidPokemon': 'Uno de los Pokémon elegidos no es válido.',
  'auth.error.usernameTaken': 'Ese nombre de usuario ya está en uso.',
  'auth.error.invalidCredentials': 'Nombre de usuario o combinación de Pokémon incorrectos.',
  'auth.error.sessionStartFailed': 'No se pudo iniciar la sesión. Inténtalo de nuevo.',
  'auth.checkingSession': 'Comprobando tu carné…',

  'intro.rulesIntro': 'Forma tu equipo antes de entrar. Las reglas son estrictas:',
  'intro.rule.noItems': 'Sin objetos equipados',
  'intro.rule.noUsableItems': 'Sin objetos utilizables en combate',
  'intro.rule.pokemonCount': 'Solo {{count}} Pokémon',
  'intro.rule.exclusiveStarter': 'Algunos Pokémon son excluyentes entre sí: solo puedes tener un inicial',
  'intro.rule.levelCap': 'Nivel máximo: {{cap}}',
  'intro.rule.evoStage': 'Elige cualquier etapa evolutiva legal a nivel {{cap}}',
  'intro.rule.moves': 'Elige hasta {{max}} movimientos legales a nivel {{cap}}',
  'intro.cta': 'Forma tu equipo',

  'leader.brock.splashTitle': 'Breaking Brock',
  'leader.brock.heading': 'Gimnasio Plateado',
  'leader.brock.rivalLabel': 'Líder de Gimnasio Brock',
  'leader.brock.description':
    'Dentro te espera **Brock**, el Líder de Gimnasio de tipo Roca: duro, paciente, y a punto de comprobar si tu equipo aguanta un golpe de verdad.',
  'leader.brock.name': 'Brock',

  'leader.misty.splashTitle': 'Misty Waters',
  'leader.misty.heading': 'Gimnasio Celeste',
  'leader.misty.rivalLabel': 'Líder de Gimnasio Misty',
  'leader.misty.description':
    'Dentro te espera **Misty**, la Líder de Gimnasio de tipo Agua: rápida, implacable, y lista para arrasar con cualquier equipo que no logre mantener el equilibrio.',
  'leader.misty.name': 'Misty',

  'leaderBar.locked': 'Aún no disponible',
  'leaderBar.lockedInBuild': 'Vuelve atrás para cambiar de líder',

  'teamBuilder.loadingRoster': 'Cargando Pokédex…',
  'teamBuilder.loadRosterFailed': 'No se pudo cargar la Pokédex.',
  'teamBuilder.back': 'Volver',
  'teamBuilder.heading': 'Forma tu equipo',
  'teamBuilder.importFromShowdown': 'Importar desde Showdown',
  'teamBuilder.cancelImport': 'Cancelar importación',
  'teamBuilder.rules': 'Nivel máximo {{cap}} · Sin objetos · Solo {{count}} Pokémon · elige una etapa evolutiva y hasta {{max}} movimientos legales a nivel {{cap}}.',
  'teamBuilder.importPlaceholderIntro': 'Pega una exportación de Showdown, p. ej.',
  'teamBuilder.import': 'Importar',
  'teamBuilder.importing': 'Importando…',
  'teamBuilder.importFailed': 'No se pudo importar el equipo.',
  'teamBuilder.pokemonSlot': 'Pokémon {{n}}',
  'teamBuilder.selectHeading': 'Elige el Pokémon {{n}}',
  'teamBuilder.slotLocked': 'Bloqueado para este desafío',
  'teamBuilder.removePokemon': 'Quitar Pokémon',
  'teamBuilder.starterBlocked': 'Solo se permite un inicial en tu equipo',
  'teamBuilder.tradeBlocked': 'Cambiarlo por otro te cuesta el que ya tienes',
  'teamBuilder.duplicateBlocked': 'Tu equipo no puede tener el mismo Pokémon dos veces.',
  'teamBuilder.familyBlocked': 'Ya está en tu equipo como otra etapa evolutiva',
  'teamBuilder.starterValidation': 'Solo puedes tener un inicial (Bulbasaur/Charmander/Squirtle) en tu equipo.',
  'teamBuilder.tradeValidation': 'No puedes tener ambos lados de un intercambio del juego (p. ej., Clefairy y Mr. Mime) en tu equipo.',
  'teamBuilder.familyValidation': 'Tu equipo no puede tener dos Pokémon de la misma familia evolutiva (p. ej., Gloom y Vileplume).',
  'teamBuilder.abilityHeading': 'Habilidad',
  'teamBuilder.natureHeading': 'Naturaleza',
  'teamBuilder.selectNature': 'Elige naturaleza',
  'teamBuilder.movesHeading': 'Movimientos',
  'teamBuilder.movesSelected': '{{count}} / {{max}} seleccionados',
  'teamBuilder.battleCta': '¡A luchar!',

  'battle.loading': '{{leader}} está sacando a su equipo…',
  'battle.runFailed': 'No se pudo ejecutar la batalla.',
  'battle.backToBuilder': 'Volver a formar equipo',
  'battle.step': 'Siguiente movimiento',
  'battle.skipToEnd': 'Saltar al final',
  'battle.pause': 'Pausa',
  'battle.play': 'Reproducir',
  'battle.newTeam': 'Formar un nuevo equipo',
  'battle.rematch': 'Luchar de nuevo',
  'battle.turnHeading': 'Turno {{n}}',
  'battle.turnHeading0': 'Salida a combate',
  'battle.outcome.tie': 'La batalla terminó en empate.',
  'battle.outcome.win': '¡Has derrotado a {{leader}}!',
  'battle.outcome.lose': '{{leader}} ha derrotado a tu equipo.',
  'battle.playerLabel': 'Rojo',

  'battle.entersBattleSuffix': ' entra en combate!',
  'battle.abilityActivatedSuffix': ' activó {{ability}}!',
  'battle.weatherBegins': '¡Comienza: {{weather}}!',
  'battle.used': 'usó',
  'battle.on': 'en',
  'battle.targetSelf': 'sí mismo',
  'battle.targetAlly': 'su aliado',
  'battle.targetAllFoes': 'todos los rivales',
  'battle.targetAllAdjacent': 'todos los cercanos',
  'battle.targetField': 'todos',
  'battle.targetFoeSide': 'el lado rival',
  'battle.targetOwnSide': 'su lado',
  'battle.targetTeam': 'su equipo',
  'battle.resistedPrefix': 'No es muy eficaz contra',
  'battle.resistedSuffix': '...',
  'battle.superEffectivePrefix': '¡Es muy eficaz contra',
  'battle.superEffectiveSuffix': '!',
  'battle.immuneSuffix': ' es inmune a ese movimiento.',
  'battle.critPrefix': '¡Un golpe crítico a',
  'battle.critSuffix': '!',
  'battle.missPrefix': 'El ataque de',
  'battle.missSuffix': ' falló!',
  'battle.failTargetPrefix': 'El movimiento de',
  'battle.failTargetSuffix': ' falló!',
  'battle.failGeneric': '¡Pero falló!',
  'battle.tookDamageSuffix': ' recibió daño ({{amount}}).',
  'battle.restoredHpSuffix': ' recuperó PS ({{amount}}).',
  'battle.hpUnit': 'PS',
  'battle.boostRose': 'subió',
  'battle.boostFell': 'bajó',
  'battle.statusWas': 'quedó',
  'battle.status.par': 'paralizado',
  'battle.status.brn': 'quemado',
  'battle.status.psn': 'envenenado',
  'battle.status.tox': 'gravemente envenenado',
  'battle.status.slp': 'dormido',
  'battle.status.frz': 'congelado',
  'battle.status.generic': 'afectado por un estado alterado',
  'battle.curedSuffix': ' se recuperó de su estado!',
  'battle.faintedSuffix': ' se debilitó!',
  'battle.winsSuffix': ' gana la batalla!',
  'battle.tieLine': '¡La batalla terminó en empate!',

  'battle.moveDetail.power': 'Potencia',
  'battle.moveDetail.accuracy': 'Precisión',
  'battle.moveDetail.pp': 'PP',
  'battle.moveDetail.priority': 'Prioridad',
  'battle.moveDetail.error': 'No se pudo cargar el movimiento.',

  'battle.reportMove.button': 'Reportar este movimiento',
  'battle.reportMove.title': 'Sugerir un movimiento mejor',
  'battle.reportMove.suggestionLabel': '¿Qué debería haber pasado en su lugar?',
  'battle.reportMove.suggestionPlaceholder': 'p. ej. Cambiar al otro Pokémon, o usar otro movimiento…',
  'battle.reportMove.reasonLabel': '¿Por qué?',
  'battle.reportMove.reasonPlaceholder': 'p. ej. Ese movimiento fue resistido, un compañero podría haber noqueado…',
  'battle.reportMove.submit': 'Enviar sugerencia',
  'battle.reportMove.thanks': '¡Gracias! Tu sugerencia fue registrada.',
  'battle.reportMove.error': 'No se pudo enviar la sugerencia.',

  'footer.cta.0': '¿Un fallo? Cuéntame cada detalle, ¡por la ciencia!',
  'footer.cta.1': '¿Algo no funciona bien? ¡Repórtalo como un buen investigador!',
  'footer.cta.2': 'Hasta mi mejor investigación tiene fallos. ¡Ayúdame a corregirlo!',
  'footer.cta.3': '¿Disfrutando tu aventura? Cuéntamelo, ¡alegra mi día!',
  'footer.cta.4': 'Una palabra amable hace maravillas en el laboratorio. ¡Dila!',
  'footer.cta.5': 'Ya sea que te hizo reír o suspirar, ¡necesito saberlo!',
  'footer.cta.6': '¿Una idea brillante? ¡Mi cuaderno siempre tiene espacio para una más!',
  'footer.cta.7': '¿Se te ocurrió una nueva función? ¡Soy todo oídos!',
  'footer.cta.8': '¿Qué haría esta aventura aún mejor? ¡Ilumíname!',
  'footer.cta.9': 'Dos segundos, una idea: ¡complace a este viejo profesor!',
  'footer.button': 'Dejar un comentario',
  'footer.placeholder': 'Escribe tu comentario aquí…',
  'footer.submit': 'Enviar comentario',
  'footer.thanks': '¡Gracias! Tu comentario fue registrado.',
  'footer.error': 'No se pudo enviar el comentario.',
  'footer.close': 'Cerrar',

  'pokemonCard.natureNeutral': 'Neutral',
  'pokemonCard.itemHeading': 'Objeto equipado',
  'pokemonCard.itemNone': 'Ninguno',
  'pokemonCard.statsHeading': 'Estadísticas base',
  'pokemonCard.viewDetails': 'Ver detalles de {{name}}',
};

export const translations = { en, es };
export type TranslationKey = keyof typeof en;

export function interpolate(template: string, vars?: Record<string, string | number>): string {
  if (!vars) return template;
  return template.replace(/\{\{(\w+)\}\}/g, (_, key: string) => String(vars[key] ?? ''));
}
