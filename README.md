...
Gen 9 double-battle simulator, built on [`@pkmn/sim`](https://github.com/pkmn/ps) (the real Pokémon Showdown simulator engine), with a web frontend for picking a FireRed/LeafGreen gym leader — Brock, Misty, and Lt. Surge are playable today — building a team legal for that leader (species obtainable before their gym, at their own level cap and team size), and auto-battling it against their fixed team.
...
- `src/config/leaders/` — the registry of gym leaders (`index.ts`: `getLeader`, `listLeaders`, `DEFAULT_LEADER_ID`) and the `LeaderConfig`/`LeaderRules` shape (`types.ts`) each one is defined against — team size, level cap, base species pool, evolution items, ace index, thematic type. Brock, Misty, and Lt. Surge are `available: true`; the rest are bare placeholders.
- `src/config/teams/` — team definitions (Pokemon Showdown export-text format). `fireRed/brock.ts`, `fireRed/misty.ts`, and `fireRed/ltSurge.ts` are each leader's fixed team, referenced from their `LeaderConfig`; `player.ts` is a CLI-only placeholder team, also used by `npm run simulate`.
- `src/roster/roster.ts` — derives the frontend's selectable Pokemon pool per leader (every species obtainable in FireRed/LeafGreen before that leader's gym — see `scripts/pokemon-before-brock.ts` / `pokemon-before-misty.ts` / `pokemon-before-lt-surge.ts`) from `@pkmn/sim`'s own `Dex`: which evolution stages are reachable at or below that leader's level cap (plus any item-gated evolution their `evolutionItems` allows, e.g. the Moon Stone unlocked from Misty on), and which level-up moves are legal at that cap, including moves learned earlier in the evolution line. Also serves natures and single-move lookups.
...
- Level cap follows the chosen leader (13 for Brock, 19 for Misty, 21 for Lt. Surge) — evolution stage and moveset must be legal at or below it
